...
- `git push` 成功返回时，HEAD 读视图和文章缓存已经同步完成，无需重启服务或重新构建前端
- 内容仓库工作区必须保持干净；服务使用 Git `updateInstead`，检测到服务端未提交修改时会拒绝 push，避免覆盖内容
- 页面内容、About Me 和图片都从当前 HEAD 读取；服务器工作区里的手工草稿不会对外展示
