...
- **终端风格 UI**：采用 Dracula 配色方案，提供沉浸式终端体验
...
| `ls [options]` | 列出当前目录内容 | `ls` |
| `ls --sort=created` | 按创建时间排序（升序） | `ls --sort=created` |
| `ls --sort=edited` | 按编辑时间排序（降序，默认） | `ls --sort=edited` |
| `ls --sort=created --desc` | 按创建时间降序 | `ls --sort=created --desc` |
...
**排序规则**：
- 默认排序：按最后编辑时间降序（最近更新在前）
- 支持排序字段：`created`（创建时间）、`edited`（编辑时间）
- 支持排序方向：默认降序，`--asc` 升序，`--desc` 降序
...
│  [展开] 编辑时间线                                      │
│  ┌─────────────────────────────────────────────┐   │
│  │ commit a1b2c3d | 2024-01-15 10:30 | 作者A    │   │
│  │ commit e4f5g6h | 2024-01-14 15:20 | 作者B    │   │
│  │ commit i7j8k9l | 2024-01-10 09:00 | 作者A    │   │
│  └─────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────┘
```
...
| 端点 | 方法 | 描述 |
|------|------|------|
| `/api/articles` | GET | 获取文章列表，支持 `?sort=created\|edited&order=asc\|desc` |
| `/api/articles/{path}` | GET | 获取文章内容 |
| `/api/articles/{path}/timeline` | GET | 获取文章 Git 时间线 |
| `/api/tree` | GET | 获取目录树结构 |
| `/api/search` | GET | 搜索文章标题（query: `?q=keyword`） |
...
- [ ] 编辑时间线正确显示 commit 历史（短 hash、时间、作者）
- [ ] 未提交文件不显示在 Blog 中
- [ ] 标题搜索功能正常
- [ ] 文章排序功能正常（默认 + 命令行参数）
