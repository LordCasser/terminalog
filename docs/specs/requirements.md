...
| 文件成功 push | push 成功响应返回后，新的文章、目录、内容与资源立即可见 |
| 文件删除 | 从 Blog 中移除 |
...
2. **搜索命令**：
   ...
   - 后端检索匹配文章标题、文件名和目录名（支持中文子串匹配），返回匹配的文章路径列表
   - **过滤约束**：不搜索以 `_` 开头的隐藏文件（如 `_ABOUTME.md`）
   - **搜索结果统一模态框**：无论搜索结果数量（1个或多个），均弹出模态框让用户选择后跳转，不再对单个结果直接跳转
   - **中文支持**：搜索关键词支持中文子串匹配（如搜索"体系"可匹配"微体系结构攻击"）
...
**搜索结果模态框（v1.6新增，v1.7修改）**：
1. **统一模态框**：无论搜索返回1个还是多个结果，均弹出模态框显示搜索结果列表，用户选择后跳转
2. **结果显示**：
   - 左对齐显示文章标题
   - 右对齐显示文章最后更新日期
3. **交互方式**：
...
**Tab键路径补全模态框（v1.6新增，v1.7修改）**：
1. **指令补全**：Tab键自动补全指令名称（如`se`→`search `），单匹配直接填充
2. **search 不可补全**：`search` 命令的参数不参与 Tab 补全逻辑，按Tab时显示"search 不可补全"提示
3. **路径补全模态框**：当`open`或`cd`命令后路径有多个匹配时：
   - 弹出模态框显示所有匹配路径
   - 左侧显示文件类型图标（📄文件/📁文件夹）
   - 文件夹路径带斜杠结尾
4. **交互方式**：
...
4. **search 不可补全**：在`search`命令后按Tab键时，显示1秒提示"search 不可补全"，不触发路径补全
5. **提示样式**：显示在输入框右上角上方，不遮挡输入内容，使用error颜色，1秒后自动消失
