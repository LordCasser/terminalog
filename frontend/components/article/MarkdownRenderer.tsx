...
function extractHeadings(markdown: string): HeadingEntry[] {
  ...
  while ((match = headingRegex.exec(withoutCode)) !== null) {
    ...
    const slug = rawText
      .toLowerCase()
      .replace(/[^\w\u4e00-\u9fff\s]/g, '')   // Remove non-word, non-CJK, non-space (match rehype-slug)
 ...
 * Only replaces when an explicit [TOC] or [toc] marker is present on its own line.
 * Returns cleaned content (marker removed) and extracted headings for separate TOC rendering.
 */
function useTocProcessing(content: string): { cleanContent: string; tocHeadings: HeadingEntry[] | null } {
  return useMemo(() => {
    const tocRegex = /^\[toc\]$/im;
    if (!tocRegex.test(content)) {
      return { cleanContent: content, tocHeadings: null };
    }
    const headings = extractHeadings(content);
    const cleanContent = content.replace(tocRegex, '');
    return { cleanContent, tocHeadings: headings };
  }, [content]);
}
...
export function MarkdownRenderer({ content, basePath }: MarkdownRendererProps) {
  // Pre-process [TOC] markers. TOC is rendered as a separate React component,
  // bypassing fragile virtual-DOM introspections in the ul renderer.
  const { cleanContent, tocHeadings } = useTocProcessing(content);

  const tocMinLevel = tocHeadings ? Math.min(...tocHeadings.map(h => h.level)) : 0;

  return (
    <div className="markdown-body">
      {/* Auto-generated Table of Contents — only when [TOC] marker is present */}
