...
CommandPrompt 是终端风格的命令输入组件，固定在页面底部，提供命令行交互体验。支持Tab键自动补全命令（禁用浏览器默认Tab键行为）。
...
### 2. 键盘聚焦机制
- **全局键盘监听**：按下任意键自动聚焦输入框（包括Tab键）
- **Tab键特殊处理**：禁用浏览器默认Tab键行为（焦点切换），用于命令补全
- **忽略特殊键**：导航键（Arrow）、功能键（F1-F12）、修饰键（Meta, Ctrl, Alt）
- **智能聚焦**：仅在未聚焦状态下响应键盘事件
